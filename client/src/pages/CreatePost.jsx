...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      ...
      //   const data = await axios.post("http://localhost:8080/api/v1/dalle", { prompt: form.prompt }, headers);
      //   setForm({ ...form, photo:`data:image/jpeg;base64,${data.photo} `})
      } catch (err) {
        alert(err);
        console.log(err);
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      alert("Please provide proper prompt and photo");
    }
  };
    ...
    <section className="max-w-7xl mx-auto">
      ...
      focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center"
        >
          ...
          {generatingImage && (
            <div className="absolute inset-0 z-0 flex justify-center items-center bg-[rgba(0,0,0,0.5)] opacity-40 rounded-lg ">
              <Loader />
            </div>
          )}
        </div>
        ...
        <div className="mt-10">
          <p className="mt-2 text-[#666e75] text-[14px]">
            Once you have created image, if you want you can share it in
            community{" "}
          </p>
        </div>
        <button
